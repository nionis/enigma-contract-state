...
contract("Sample", accounts => {
  ...
  it("stateBool should be false", async () => {
    const contract = await SampleContract.at(SampleContract.address)
    const stateBool = await contract.stateBool();

    expect(stateBool).to.equal(false);
  })
  ...
  it("stateBool should be true", async () => {
    const contract = await SampleContract.at(SampleContract.address)
    const stateBool = await contract.stateBool();

    expect(stateBool).to.equal(true);
  })
})
